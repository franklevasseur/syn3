...
export type TokenType = 'opening-bracket' | 'closing-bracket' | 'white-space' | 'text'
...
const sortRanges = <R extends RawToken>(ranges: R[]) => _.sortBy(ranges, (u: RawToken) => u.start)

const computeRemaining = (original: RawToken, extracted: RawToken[]): RawToken[] => {
...
const extractSingleUnit = (unit: RawToken, type: TokenType, regx: () => RegExp): ExtractionResult => {
  const matcher = regx()
...
const extractRegex = (units: RawToken[], tokType: TokenType, regx: () => RegExp): ExtractionResult => {
  let extracted: Token[] = []
  let remaining: RawToken[] = []

  for (const u of units) {
    const { extracted: singleExtraction, remaining: singleRemaining } = extractSingleUnit(u, tokType, regx)
...
export const tokenize = (input: string): Token[] => {
  ...
  const { extracted: extractedOpeningBrackets, remaining: remainingOpeningBrackets } = extractRegex(
    initial,
    'opening-bracket',
    () => OPENING_BRACKET_REGX,
  )

  const { extracted: extractedClosingBrackets, remaining: remainingClosingBrackets } = extractRegex(
    remainingOpeningBrackets,
    'closing-bracket',
    () => CLOSING_BRACKET_REGX,
  )

  const { extracted: extractedWhiteSpaces, remaining: remainingWhiteSpaces } = extractRegex(
  ...
  return sortRanges([...extractedOpeningBrackets, ...extractedClosingBrackets, ...extractedWhiteSpaces, ...words])
}
